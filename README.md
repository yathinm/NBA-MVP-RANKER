...
```
NBA-MVP-RANKER/
├── NBAMVPSORTER.py                    # Original Python script
├── NBAMVPSORTEREnhanced.py            # Enhanced Python script with advanced analytics
├── MVPViewer.java                     # Original Java GUI
├── MVPViewerEnhanced.java             # Enhanced Java GUI with modern styling
├── MVPViewer.class                    # Compiled original Java bytecode
├── MVPViewerEnhanced.class            # Compiled enhanced Java bytecode
├── requirements.txt                   # Python dependencies
├── NBA_2024_per_game.csv             # Input NBA statistics data
├── mvp_candidates.csv                 # Basic output (original script)
├── mvp_candidates_enhanced.csv        # Enhanced output with multiple scores
├── mvp_analysis.json                  # Comprehensive analysis results
├── mvp_analysis_dashboard.png         # Data visualization dashboard
├── demo.py                           # Demo script to showcase features
└── README.md                         # This file
```
...
The enhanced Python script allows easy customization of all scoring methods:

```python
# Adjust composite score weights
self.df['MVP_Score_Composite'] = (
    self.df['MVP_Score_Basic_Normalized'] * 0.3 +      # Adjust basic weight
    self.df['MVP_Score_Advanced_Normalized'] * 0.3 +   # Adjust advanced weight
    self.df['MVP_Score_Position_Normalized'] * 0.2 +   # Adjust position weight
    self.df['MVP_Score_Team_Impact_Normalized'] * 0.2  # Adjust team impact weight
)
```

### Changing Filters and Thresholds

```python
# Modify minimum games requirement
self.df = self.df[self.df['G'] >= 25]  # Change from 20 to 25 games

# Add additional filters
self.df = self.df[self.df['MP'] >= 1000]  # Minimum minutes played
```
...
1. **"File not found" error**: Ensure `NBA_2024_per_game.csv` is in the same directory
2. **Java compilation error**: Verify Java 8+ is installed and in your system PATH
3. **Python import errors**: Install dependencies with `pip install -r requirements.txt`
4. **Visualization errors**: Install matplotlib/seaborn with `pip install matplotlib seaborn`
5. **GUI not displaying**: Ensure `mvp_candidates_enhanced.csv` was generated first

### System Requirements

- **Python**: 3.7 or higher
- **Java**: 8 or higher
- **Memory**: Minimum 1GB RAM (2GB recommended for visualizations)
...
- **Real-time Data**: NBA API integration for live statistics
- **Machine Learning**: Predictive models for MVP voting
- **Advanced Analytics**: PER, VORP, BPM calculations
...
- **Algorithm Development**: New scoring methodologies
